...
for state in config["states"]:
    ...
    for figure_data in config["figures"]:
        ...
        for chart in figure_data["charts"]:
            ...
            scatter = marker(
                x=[0], y=[0], color=chart["color"], size=10, legend=chart["legend"]
            )
            line = new_figure.line(
                x=[0], y=[0], color=chart["color"], line_width=1, legend=chart["legend"]
            )
            # adding data source for markers and line
            data_sources[state][chart["key"]] = scatter.data_source = line.data_source
        new_figure.legend[0].location = "top_left"
        figures.append(new_figure)

requests_key = config["requests_key"]
url = config["url"]

# Create gridplot containing 2 figures in row for each state
layout = gridplot(figures, ncols=2)

curdoc().add_root(layout)


# Define and add a periodic callback to be run every 1 second:
def update():
    try:
